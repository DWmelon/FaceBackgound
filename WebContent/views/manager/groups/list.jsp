        ...
        <div id="main-title">
          ...
          <ul class="title-nav nav nav-pills">
           ...
           <form style="text-align: right;" action="list" method="post">
             <input type="text" name="search" class="input-medium" placeholder="查询分组" value="${search}"/>
             <input id="btnSubmit" class="btn btn-primary" type="submit" value="查询"/>
           </form>
           <table class="table table-striped table-bordered table-condensed">
             <thead><tr><th>分组编码</th><th>分组名</th><th>操作</th></tr></thead>
             <tbody>
             <c:forEach items="${list}" var="groups">
               <tr>
                 <td><a  href="${ctx}/groups/edit?id=${groups.id}">${groups.number}</a></td>
                 <td>${groups.name}</td>
                 <td>
                   <a href="${ctx}/groups/edit?id=${groups.id}">修改</a>
                   <a href="#" onclick="if (confirm('确认要删除分组吗？此操作将不可恢复！'))
                           location = '${ctx}/groups/delete?id=${groups.id}';
                           return false;">删除</a>
                 </td>
               </tr>
             </c:forEach>
