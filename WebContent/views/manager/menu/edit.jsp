      ...
      <div id = "wrap" class = "container-fluid">
        ...
        <div class = " container main-defined" id ="main">
         ...
         <div class="col-main col-md-10">
           <div id="main-title">
             <h4 class="title">菜单项管理</h4>
             <ul class="title-nav nav nav-pills">
               <li>
                 <a href="list">菜单列表</a>
               </li>
               <li class="active">
                 <a href="#">${menu.id>0 ? "修改" : "添加"}菜单${menu.id>0 ? "（注：修改菜单将会删除角色的对应的所有依赖项）" : ""}</a>
               </li>
             </ul>
           </div>
           <div id="messageBox" class="alert alert-${type} ${empty message ? 'hide' : ''}">
            ...
            <form name="menu" method="post" action="edit" style="width: 40%" >
              <input name="id" type="hidden" value="${menu.id}">
              <input name="oldName" type="hidden" value="${menu.name}">
              <div class="form-group">
                <label>编码</label>
                <input name="number" type="text" class="form-control" value="${menu.number}" required>
              </div>
              <div class="form-group">
                <label>菜单名</label>
                <input name="name" type="text" class="form-control" value="${menu.name}" required>
              </div>
              <div class="form-group">
                <label>排序号</label>
                <input name="orderId" type="text" class="form-control" value="${menu.orderId}" required>
              </div>
              <div class="form-group">
                <label>父菜单</label>
                <select name="parentId">
                  <option value="0" ${menu.parentId eq null ? 'selected':''}>无</option>
                  <c:forEach items="${parentMenus}" var="parentMenu">
                    <option value="${parentMenu.id}" ${parentMenu.id eq menu.parentId ? 'selected':''}>${parentMenu.name}</option>
                  </c:forEach>
                </select>
              </div>
              <div >
                <span>是否子菜单${menu.id>0 ? "（若此菜单含有子菜单，修改为子菜单将会失败）" : ""}
                <input id ="leaf" name="leaf" type="checkbox" class="form-control" ${menu.isLeaf eq 1 ? 'checked' : ''}  ></span>
              </div>
              <div id="permission">菜单权限项：
              <c:forEach items="${permissionList}" var="permission" varStatus="status">
                <div><input id="permissions" type="checkbox" name="pers" value="${permission.id}" ${permissionsSelected[status.index] ? 'checked':''}>${permission.name}</div>
              </c:forEach>
              </div>
