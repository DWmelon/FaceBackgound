        ...
        <div id="main-title">
          ...
          <ul class="title-nav nav nav-pills">
           ...
           <form style="text-align: right;" action="list" method="post">
             <input type="text" name="search" class="input-medium" placeholder="查询菜单" value="${search}"/>
             <input id="btnSubmit" class="btn btn-primary" type="submit" value="查询"/>
           </form>
           <table class="table table-striped table-bordered table-condensed">
             <thead><tr><th>菜单编码</th><th>菜单名</th><th>排序号</th><th>是否子菜单</th><th>操作</th></tr></thead>
             <tbody>
             <c:forEach items="${list}" var="menu">
               <tr>
                 <td><a  href="${ctx}/menu/edit?id=${menu.id}">${menu.number}</a></td>
                 <td>${menu.name}</td>
                 <td>${menu.orderId}</td>
                 <td>${menu.isLeaf==1 ? '√':'×'}</td>
                 <td>
                   <a href="${ctx}/menu/edit?id=${menu.id}">修改</a>
                   <a href="#" onclick="if (confirm('确认要删除菜单吗？此操作将不可恢复！'))
                           location = '${ctx}/menu/delete?id=${menu.id}';
                           return false;">删除</a>
                 </td>
               </tr>
             </c:forEach>
