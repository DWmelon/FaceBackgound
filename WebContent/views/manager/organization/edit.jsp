      ...
      <div id = "wrap" class = "container-fluid">
        ...
        <div class = " container main-defined" id ="main">
         ...
         <div class="col-main col-md-10">
           <div id="main-title">
             <h4 class="title">组织项管理</h4>
             <ul class="title-nav nav nav-pills">
               <li>
                 <a href="list">组织列表</a>
               </li>
               <li class="active">
                 <a href="#">${organization.id>0 ? "修改" : "添加"}组织</a>
               </li>
             </ul>
           </div>
           <div id="messageBox" class="alert alert-${type} ${empty message ? 'hide' : ''}">
            ...
            <form name="organization" method="post" action="edit" style="width: 40%" >
              <input name="id" type="hidden" value="${organization.id}">
              <input name="oldName" type="hidden" value="${organization.name}">
              <div class="form-group">
                <label>编码</label>
                <input name="number" type="text" class="form-control" value="${organization.number}" required>
              </div>
              <div class="form-group">
                <label>组织名</label>
                <input name="name" type="text" class="form-control" value="${organization.name}" required>
              </div>
              <div class="form-group">
                <label>父组织</label>
                <select name="parentId">
                  <option value="0" ${organization.parentId eq null ? 'selected':''}>无</option>
                  <c:forEach items="${parentOrg}" var="parentOrg">
                      <option value="${parentOrg.id}" ${parentOrg.id eq organization.parentId ? 'selected':''}>${parentOrg.name}</option>
                  </c:forEach>
                </select>
              </div>
              <div >
                <span>是否子组织${organization.id>0 ? "（若此组织含有子组织，修改为子组织将会失败）" : ""}
                <input name="leaf" type="checkbox" class="form-control" ${organization.isLeaf eq 1 ? 'checked' : ''}  ></span>
              </div>
              <div class="form-group">
                <input type="radio" name="orgType" value="1" ${organization.orgType != 2 ? 'checked' : ''}> 公司
                <input type="radio" name="orgType" value="2" ${organization.orgType == 2 ? 'checked' : ''}> 部门
              </div>
