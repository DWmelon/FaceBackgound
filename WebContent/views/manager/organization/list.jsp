        ...
        <div id="main-title">
          ...
          <ul class="title-nav nav nav-pills">
           ...
           <form style="text-align: right;" action="list" method="post">
             <input type="text" name="search" class="input-medium" placeholder="查询组织" value="${search}"/>
             <input id="btnSubmit" class="btn btn-primary" type="submit" value="查询"/>
           </form>
           <table class="table table-striped table-bordered table-condensed">
             <thead><tr><th>组织编码</th><th>组织名</th><th>组织级别</th><th>组织类型</th><th>操作</th></tr></thead>
             <tbody>
             <c:forEach items="${list}" var="organization">
               <tr>
                 <td><a href="${ctx}/organization/edit?id=${organization.id}">${organization.number}</a></td>
                 <td>${organization.name}</td>
                 <td>${organization.level}</td>
                 <td>${organization.orgType==1 ?'公司':'部门'}</td>
                 <td>
                   <a href="${ctx}/organization/edit?id=${organization.id}">修改</a>
                   <a href="#" onclick="if (confirm('确认要删除组织(删除父组织将会删除对应的子组织)吗？此操作将不可恢复！'))
                           location = '${ctx}/organization/delete?id=${organization.id}';
                           return false;">删除</a>
                 </td>
               </tr>
             </c:forEach>
