        ...
        <div class = " container main-defined" id ="main">
         ...
         <div class="col-main col-md-10">
           ...
           <div id="messageBox" class="alert alert-${type} ${empty message ? 'hide' : ''}">
            ...
            <form name="role" method="post" action="edit" style="width: 40%" >
              <input name="id" type="hidden" value="${role.id}">
              <input name="oldName" type="hidden" value="${role.name}">
              <div class="form-group">
                <label>编码</label>
                <input name="number" type="text" class="form-control" value="${role.number}" required>
              </div>
              <div class="form-group">
                <label>角色名</label>
              ...
              <div>菜单权限分配</div>
              <c:forEach items="${menuPermissions}" var="menuPermission" varStatus="status">
                <label>${menuPermission.menu.name}</label>
                <c:forEach items="${menuPermission.permissionList}" var="permission" varStatus="status">
                  <div> <input name="menuPermissions" type="checkbox" value="${menuPermission.menu.id},${ permission.id}" ${menuPermission.permissionSelected[status.index] ? 'checked':''}> ${permission.name}</div>
                </c:forEach>
              </c:forEach>
