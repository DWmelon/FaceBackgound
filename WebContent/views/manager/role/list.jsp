        ...
        <div id="main-title">
          ...
          <ul class="title-nav nav nav-pills">
           ...
           <form style="text-align: right;" action="list" method="post">
             <input type="text" name="search" class="input-medium" placeholder="查询角色" value="${search}"/>
             <input id="btnSubmit" class="btn btn-primary" type="submit" value="查询"/>
           </form>
           <table class="table table-striped table-bordered table-condensed">
             <thead><tr><th>角色编码</th><th>角色名</th><th>操作</th></tr></thead>
             <tbody>
             <c:forEach items="${list}" var="role">
               <tr>
                 <td><a  href="${ctx}/role/edit?id=${role.id}">${role.number}</a></td>
                 <td>${role.name}</td>
                 <td>
                   <a href="${ctx}/role/edit?id=${role.id}">修改</a>
                   <a href="#" onclick="if (confirm('确认要删除该角色吗？此操作将不可恢复！'))
                           location = '${ctx}/role/delete?id=${role.id}';
                           return false;">删除</a>
                 </td>
               </tr>
             </c:forEach>
