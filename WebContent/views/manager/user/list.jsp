        ...
        <div id="main-title">
          ...
          <ul class="title-nav nav nav-pills">
           ...
           <form style="text-align: right;" action="list" method="post">
             <input type="text" name="search" class="input-medium" placeholder="查询用户" value="${search}"/>
             <input id="btnSubmit" class="btn btn-primary" type="submit" value="查询"/>
           </form>
           <table class="table table-striped table-bordered table-condensed">
             <thead><tr><th>用户编码</th><th>用户名</th><th>所属组织</th><th>生效时间</th><th>失效时间</th><th>错误次数</th><th>是否锁定</th><th>锁定时间</th><th>姓名</th><th>AD域账号</th><th>操作</th></tr></thead>
             <tbody>
             <c:forEach items="${list}" var="user" varStatus="status">
               <tr>
                 <td><a  href="${ctx}/user/edit?id=${user.id}">${user.number}</a></td>
                 <td>${user.account}</td>
                 <td>${orgNames[status.index]}</td>
                 <td>${user.effectiveDate}</td>
                 <td>${user.invalidationDate == null ? '无':user.invalidationDate}</td>
                 <td>${user.errCount}</td>
                 <td>${user.locked > 0 ? '是':'否'}</td>
                 <td>${user.lockedTime == null ? '无':user.lockedTime}</td>
                 <td>${user.name}</td>
                 <td>${user.adAccount}</td>
                 <td>
                   <a href="${ctx}/user/edit?id=${user.id}">修改</a>
                   <a href="#" onclick="if (confirm('确认要删除该用户吗？此操作将不可恢复！'))
                           location = '${ctx}/user/delete?id=${user.id}';
                           return false;">删除</a>
                 </td>
               </tr>
             </c:forEach>
